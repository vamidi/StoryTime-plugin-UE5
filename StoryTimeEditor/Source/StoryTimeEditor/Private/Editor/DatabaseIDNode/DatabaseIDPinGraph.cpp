...
#include "Editor/DatabaseIDNode/DatabaseIDPinGraph.h"
...
void SDatabaseIDPinGraph::CreateEntryDropdownValues()
{
	...
	// Try to determine entry type
	/*
	int EntryType = 0; // 0:string, 1:double, 2:bool
	const FTableField* TypeField = DatabaseSyncModule::Get().GetField(*TableNames[TableID], *ColumnNames[ColumnID], 0);
	if (TypeField->Size == 1)
	{
	...
	for (int j = 0; j < rowCount; j++)
	{
		...
		{
			double d;
			memcpy(static_cast<void*>(&d), Field->Data.Get(), Field->Size);

			EntryNames.Add(MakeShareable<FString>(new FString(FString::SanitizeFloat(d, 0))));
		} else if (EntryType == 2) // bool
		{
			bool b;
			memcpy(static_cast<void*>(&b), Field->Data.Get(), Field->Size);

...
static const FName DatabaseURL("https://storytime.vamidicreations.nl/dashboard/projects/-MFFSn0ysMX3c6xD9zZI/tables/");
...
TSharedRef<SWidget> STableOverviewRow::GenerateWidgetForColumn(const FName& ColumnName)
{
	...
	{
		FString url = DatabaseURL.ToString() + Item.Get()->Id;
	...
	else if (ColumnName == "Sync")
	{
		...
		{
			DatabaseSyncModule::Get().RequestTableUpdate(Item.Get()->Name);
			return FReply::Handled();
		});
	}
